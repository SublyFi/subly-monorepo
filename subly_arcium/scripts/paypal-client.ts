...
export type RecipientLookupKey = `${string}:${string}`;

export function recipientHashKeyFromParts(low: BN | bigint, high: BN | bigint): RecipientLookupKey {
  const lowBig = normalizeToBigInt(low);
  const highBig = normalizeToBigInt(high);
  return `${lowBig.toString()}:${highBig.toString()}`;
}

export function computeRecipientHash(receiver: string): {
  low: bigint;
  high: bigint;
  key: RecipientLookupKey;
} {
  const digest = createHash("sha256").update(Buffer.from(receiver)).digest();
  const low = bytesToBigIntLE(digest.subarray(0, 16));
  const high = bytesToBigIntLE(digest.subarray(16, 32));
  const key = recipientHashKeyFromParts(low, high);
  return { low, high, key };
}
...
const normalizeToBigInt = (value: BN | bigint): bigint =>
