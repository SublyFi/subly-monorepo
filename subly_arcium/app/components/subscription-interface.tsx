...
export function SubscriptionInterface() {
  ...
  const updatePayPalState = useCallback((details: PayPalRecipientDetails | null) => {
    setHasPayPal(Boolean(details?.configured))
  }, [])
  ...
  const loadYieldData = useCallback(async () => {
    ...
    try {
      ...
      const [{ summary }, { details }] = await Promise.all([
        fetchUserAvailableServices(connection, userPk, sendComputation),
        fetchPayPalRecipient(connection, userPk, sendComputation),
      ])

      updatePayPalState(details)

      const totalPrincipal = Number(formatUsdcFromSmallest(summary.totalPrincipal))
      const monthlyBudget = Number(formatUsdcFromSmallest(summary.monthlyBudgetUsdc))
      const budgetRemaining = Number(formatUsdcFromSmallest(summary.availableBudgetUsdc))

      setTotalStaked(Number.isFinite(totalPrincipal) ? totalPrincipal : 0)
      setAvailableYield(Number.isFinite(monthlyBudget) ? monthlyBudget : 0)
      setAvailableBudget(Number.isFinite(budgetRemaining) ? budgetRemaining : 0)
    } catch (error) {
  ...
  }, [activeWallet, connection, sendComputation, updatePayPalState, walletConnected])

  const loadServices = useCallback(() => {
    setServicesLoading(true)
    const mapped: SubscriptionServiceCard[] = SERVICE_CATALOG.map(
      (service: SubscriptionServiceEntry, index) => ({
        id: service.id ?? index,
        name: service.name,
  ...
  const loadUserSubscriptions = useCallback(async () => {
    ...
    try {
      const userPk = new PublicKey(activeWallet.address)
      const { view } = await fetchUserSubscriptions(connection, userPk, sendComputation)
      setUserSubscriptions(view.subscriptions)
    } catch (error) {
  ...
  }, [activeWallet, connection, sendComputation, walletConnected])
  ...
  const remainingYield = Math.max(availableBudget, 0)
  ...
  const handleSubscribe = useCallback(
    async (service: SubscriptionServiceCard) => {
      ...
      try {
        ...
        const prepared = await prepareSubscribeServiceTransaction(
          connection,
          userPk,
          service.id,
        )
  ...
  const handleUnsubscribe = useCallback(
    async (subscription: ResolvedSubscriptionCard) => {
      ...
      try {
        ...
        const prepared = await prepareUnsubscribeServiceTransaction(
          connection,
          userPk,
