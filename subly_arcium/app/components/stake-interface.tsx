...
export function StakeInterface() {
  ...
  const loadTranches = useCallback(async () => {
    ...
    try {
      ...
      const userPk = new PublicKey(activeWallet.address)
      const { entries } = await fetchUserStakeEntries(connection, userPk, sendComputation)
  ...
  }, [activeWallet, connection, resetTranches, sendComputation, walletConnected])
  ...
  const handleStake = useCallback(async () => {
    ...
    try {
      ...
      const prepared = await prepareStakeTransaction(
        connection,
        userPublicKey,
        amount,
      )
  ...
  const handleUnstake = useCallback(async () => {
    ...
    try {
      ...
      const prepared = await prepareUnstakeTransaction(
        connection,
        userPublicKey,
        selectedTrancheId,
      )
