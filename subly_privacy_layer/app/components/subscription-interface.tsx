...
import { CheckCircle, Loader2, Sparkles, XCircle, Lock } from "lucide-react";
import {
...
import { getMXEPublicKey } from "@/lib/arcium-client";
import {
  getOrCreateCipher,
  storeClientSecretKey,
...
function navigateToTab(tab: string) {
  ...
  document.dispatchEvent(
    new CustomEvent("subly:navigate-tab", { detail: { tab } })
  );
}
...
export function SubscriptionInterface() {
  ...
  const [processingServiceId, setProcessingServiceId] = useState<number | null>(
    null
  );
  const [processingUnsubscribeId, setProcessingUnsubscribeId] = useState<
    number | null
  >(null);
  ...
  const updatePayPalState = useCallback(
    (details: PayPalRecipientDetails | null) => {
  ...
  const loadYieldData = useCallback(async () => {
    ...
    try {
      ...
      const totalPrincipal = stakeEntries.reduce(
      ...
      setTotalStaked(
        Number.isFinite(principalFormatted) ? principalFormatted : 0
      );

      const annualYieldSmallest =
        (totalPrincipal * BigInt(config.apyBps)) / 10000n;
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Unable to load staking yield information."
      );
      setAvailableYield(0);
      setTotalStaked(0);
    } finally {
  ...
  const loadServices = useCallback(async () => {
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Unable to load subscription services."
      );
  ...
  const loadUserSubscriptions = useCallback(async () => {
    ...
    try {
      ...
      // Get MXE public key and create cipher for decryption
      const mxePublicKey = await getMXEPublicKey(connection, SUBLY_PROGRAM_ID);
      const { cipher } = getOrCreateCipher(activeWallet.address, mxePublicKey);

      // Fetch and automatically decrypt subscriptions
  ...
  const resolvedSubscriptions = useMemo<ResolvedSubscriptionCard[]>(() => {
    ...
    return userSubscriptions
      .map((subscription) => {
        // serviceId might be undefined if decryption failed
        ...
        const priceNumber = subscription.monthlyPrice
          ? Number(formatUsdcFromSmallest(subscription.monthlyPrice))
          : service.price;
        ...
        return {
          ...
          decrypted: !subscription.decryptionError,
          decryptionError: subscription.decryptionError,
        };
      })
  ...
  const subscribedServiceIds = useMemo(() => {
    return new Set(
      resolvedSubscriptions.map((subscription) => subscription.id)
    );
  ...
  const handleSubscribe = useCallback(
    async (service: SubscriptionServiceCard) => {
      ...
      if (!hasPayPal) {
        toast.error(
          "Add your PayPal payout information in the Profile tab before subscribing.",
          {
            action: {
              label: "Go to Profile",
      ...
      try {
        ...
        // Find the full service entry for Arcium encryption
        const services = await fetchSubscriptionServices(connection);
        ...
        // Prepare transaction with encryption
        const { transaction, blockhash, clientSecretKey, computationOffset } =
          await prepareSubscribeServiceTransaction(
            connection,
            userPk,
            fullService,
            0n // currentTotal - assuming first subscription, should be calculated from existing subscriptions
          );

        // Store client secret key for future decryption
        storeClientSecretKey(activeWallet.address, clientSecretKey);
        ...
        toast.success(`Subscribed to ${service.name}`, {
          description: `${signatureString.slice(0, 8)}…${signatureString.slice(
            -8
          )}`,
          action: {
            label: "Explorer",
  ...
  const handleUnsubscribe = useCallback(
    async (subscription: ResolvedSubscriptionCard) => {
      ...
      // Phase 4: Status is encrypted, so we can't check it client-side
      // The program will validate on-chain
      ...
      try {
        ...
        const { transaction, blockhash } =
          await prepareUnsubscribeServiceTransaction(
            connection,
            userPk,
    ...
    <div className="max-w-6xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-6 py-6 sm:py-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-0">
                ...
                <>
                  ...
                  <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                    ...
                    <span className="font-medium text-foreground">
                      {formatUsdcAmountDisplay(totalStaked)} USDC
                    </span>
                  </p>
                </>
              ) : (
            ...
            <div className="text-left sm:text-right">
              ...
              <p className="text-lg sm:text-xl font-semibold">
                {formatUsdcAmountDisplay(totalSubscriptionCost)} USDC
              </p>
              <p className="text-xs sm:text-sm text-muted-foreground">
                Remaining:{" "}
                <span className="text-green-600 font-medium">
                  {formatUsdcAmountDisplay(remainingYield)} USDC
                </span>
              </p>
            </div>
          </div>
      ...
      <Card>
        <CardHeader className="px-4 sm:px-6">
          ...
          <p className="text-sm sm:text-base text-muted-foreground">
            Subscribe to services using your staking yield
          </p>
        </CardHeader>
                ...
                {availableServices
                  ...
                  .map((service) => {
                    ...
                    return (
                      <Card
                        key={service.id}
                        className={`relative ${!canAfford ? "opacity-60" : ""}`}
                      >
                        <CardContent className="p-4 sm:p-6">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center">
                                <span className="text-lg sm:text-xl font-bold">
                                  {formatUsdcAmountDisplay(service.price)}{" "}
                                  USDC/mo
                                </span>
                              </div>
                            </div>
              ...
              {resolvedSubscriptions.length > 0 ? (
                <div className="space-y-4">
                  {resolvedSubscriptions.map((subscription) => {
                    const busy =
                      processingUnsubscribeId === subscription.subscriptionId;
                            ...
                            <div className="flex items-center justify-between sm:justify-end space-x-4">
                              <div className="text-left sm:text-right">
                                <p className="font-semibold text-sm sm:text-base">
                                  {formatUsdcAmountDisplay(subscription.price)}{" "}
                                  USDC/mo
                                </p>
