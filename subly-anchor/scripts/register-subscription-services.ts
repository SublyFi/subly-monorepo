...
async function main() {
  ...
  const registryAccount =
    await program.account.subscriptionRegistry.fetchNullable(
      subscriptionRegistryPda
    );
  const existingNames = new Set<string>(
    (registryAccount?.services ?? []).map((service: any) =>
      String(service.name).toLowerCase()
    )
  );

  console.log(
    `Loaded ${services.length} service(s) from ${path.resolve(filePath)}`
  );
